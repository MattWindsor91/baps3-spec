...
2. (Optional unless the file is either playing or not at position `0`, otherwise mandatory).
   **Broadcast** `POS` _microseconds_.
3. (Mandatory only if file playing.) **Broadcast** `STOP`.
...
1. (Optional unless the file is either playing or not at position `0`, otherwise mandatory).
   **Broadcast** `POS` _microseconds_.
2. **Unicast** `ACK` `OK` `success`.
